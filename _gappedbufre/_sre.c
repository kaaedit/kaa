...
#define DL_IMPORT(name) name
#include "../_gappedbuf/_gappedbuf.h"
#undef DL_IMPORT
...
#define SRE_MODULE "gappedbufre"
#endif

#define SRE_PY_MODULE "gappedbuf.re"
...
#define SRE_UNICODE_CHARGET(state, buf, index) \
...
sre_literal_template(int charsize, char* ptr, Py_ssize_t len)
{
    ...
    while (len-- > 0) {
        if (SRE_UNICODE_CHARGET((&state), ptr, 0) == '\\')
            return 0;
        ptr += charsize;
    }
    return 1;
...
static void *
getgappedbuf(PyObject* string, Py_ssize_t* p_length,
          int* p_logical_charsize, int* p_charsize,
          Py_buffer *view)
{
    if (PyObject_TypeCheck(string, &GappedBufferType)) {
        struct GappedBufferObj *obj =(struct GappedBufferObj*)string;
           ...
           Py_ssize_t start, Py_ssize_t end)
{
    ...
    ptr = getgappedbuf(string, &length, &logical_charsize, &charsize, &state->buffer);
    if (ptr == (void*)-1)
        goto err;
    ...
    Py_INCREF(string);
    state->string = (struct GappedBufferObj*)string;
    state->pos = start;
    state->endpos = end;
...
match_getslice_by_index(MatchObject* self, Py_ssize_t index, PyObject* def)
{
    ...
    if ((PyObject*)self->string == Py_None || self->mark[index] < 0) {
    ...
    return PySequence_GetSlice(
        (PyObject*)self->string, self->mark[index], self->mark[index+1]
        );
}
...
PyMODINIT_FUNC PyInit__gappedbufre(void)
{
    PyObject* m;
    PyObject* d;
    PyObject* x;
